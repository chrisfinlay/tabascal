...
Number of stationary RFI : {len(self.rfi_geo.keys())}"""
    ...
    def addAstro(self, I: jnp.ndarray, ra: jnp.ndarray, dec: jnp.ndarray):
        ...
        self.vis_ast += astro_vis(I_app[:, 0, 0, :].T, self.bl_uvw, lmn, self.freqs)

        n_src = I.shape[0]
        for i in range(n_src):
            self.ast_I.update({self.n_ast: I[i]})
            self.ast_lmn.update({self.n_ast: lmn[i]})
            self.ast_radec.update({self.n_ast: jnp.array([ra[i], dec[i]])})
            self.n_ast += 1
