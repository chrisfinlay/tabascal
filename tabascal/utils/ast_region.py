...
def main():
    ...
    region_path = os.path.join(os.path.split(zarr_path)[0], "astronomical_positions.ds9")
    ...
    with open(region_path, "w") as fp:
        for i in range(len(ra)):
            fp.write(CircleSkyRegion(c[i], radius=syn_bw*u.rad, visual={"color": colors[0]}).serialize(format="ds9"))