...
def extract_pow_spec(
    ms_path: str = None,
    data_col: str = "TAB_DATA",
    n_grid: int = 256,
    n_bins: int = 20,
    suffix: str = "",
):
    ...
    plt.errorbar(
        l_b, Cl_b_norm * 1e6, yerr=d_Cl_b_norm * 1e6, fmt=".", color="tab:orange"
    )
    ...
    plt.xlim(1e2, 1.5e4)
    plt.savefig(
        os.path.join(ps_dir, f"PS_Recovery_{data_col}{suffix}.pdf"),
        format="pdf",
        dpi=200,
    )
...
def main():
    ...
    parser.add_argument("-m", "--ms_path", default=None, help="Path to the MS file.")
    parser.add_argument(
        "-d",
        "--data",
    ...
    for key in data_types:
        ...
        extract_pow_spec(
            ms_path, conf["data_col"], tge_conf["n_grid"], tge_conf["n_bins"], name
        )
